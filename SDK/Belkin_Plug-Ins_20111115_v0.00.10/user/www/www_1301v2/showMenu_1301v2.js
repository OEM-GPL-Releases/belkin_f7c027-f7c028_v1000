//added by bin 04/03/2003
	var jquery;
	var chip_name="RT5350";  // For building firmware
...
var showtime = new Date().format("MMddhhmmyyyy");   //MMDDhhmmYYYY
set_client_time(showtime);
} 


function set_client_time(ClientTime)
{ 
    
   $.ajax({			
      type: "POST",
      //dataType: "script",	 //this attribute will case post.cgi can't operating properly.
      url: "SaveClientTime.cgi",			
      data: {"client_time":ClientTime},
      success: function(){
      }
  });		
}
      					...
      					'<table width="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="#CCCCCC">'+
          					'<tr>'+ 
          						'<td rowspan="3" width="20" valign="bottom">'+
          							'<a href="http://www.belkin.com" target="_blank"><img src="head_logo.gif" border="0" width="140" height="64"></a>'+
          						'</td>'+
	...
	// router or ap mode
		...
		if(menuSection=='WLAN')
		{
            ...
            if(menuItem=='Security')
    		{
         	...
         	if(menuItem=='AP_Client')
    		{
         		printMenuDisabledItem('apc_security.html', str_9999, red, darkBlue);
         	}
         	else
         	{
         		printMenuItem('apc_security.html', str_9999, black, blue);
         	}
/*
...
*/
		...
		else
		{
    		...
    		printMenuItem('apc_security.html', str_9999, black, white);
...
//	if('<%nvram_get("wan_enabled");%>' == '1')
  ...
  if (chip_name=="RT5350")
  {	    
      if(typeof(document.getElementsByName("redirect_timer")) != "undefined")
       {
            if(Number($('input[name="redirect_timer"]').val()) > 30 )
             { 
                //  $('input[name="redirect_timer"]').val(Number($('input[name="redirect_timer"]').val())+Number(5000));
             }
       }
   }
}
