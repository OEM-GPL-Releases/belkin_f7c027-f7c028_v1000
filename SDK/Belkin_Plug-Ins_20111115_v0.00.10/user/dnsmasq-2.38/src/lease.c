...
void lease_init(struct daemon *daemon, time_t now)
{
  ...
  //if (!leasestream || (rc = pclose(leasestream)) == -1 || WEXITSTATUS(rc) == 127 || WEXITSTATUS(rc) == 126)  //gmtk mark for coverity 20090804
	if (!leasestream || WEXITSTATUS(rc) == 127 || WEXITSTATUS(rc) == 126)
  {
...
//+++ gmtk add for coverity 20090804
  if(leasestream)
  {  
    rc=pclose(leasestream);      
    if(rc == -1)
    {
     if (WEXITSTATUS(rc) == 127)
	     errno = ENOENT;
