...
void tftp_request(struct listener *listen, struct daemon *daemon, time_t now)
{
  ...
  char *opt_ori=NULL;  //gmtk add for coverity 20090805
  ...
  if (ntohs(*((unsigned short *)packet)) != OP_RRQ ||
	...
	{
	  opt_ori = opt;   //gmtk add for coverity 20090805 
    ...
    if (strcasecmp(opt_ori, "tsize") == 0 && next(&p, end))  //gmtk add for coverity 20090805	  
      {
