...
static void add_txt(struct daemon *daemon, char *name, char *txt)
{
  ...
  r->txt = safe_malloc(len+2);  //gmtk add (from if to else)for coverity 20090804
  r->len = len+1;
  *(r->txt) = len;
...
{      
  ...
  free(directory);  //gmtk add (from if to else)for coverity 20090804     
  if(path)
  	free(path);  //gmtk add (from if to else)for coverity 20090804
  closedir(dir_stream);
	...
	*comma = 0;
	...
	if (!arg)
	  {
	    option = '?';
	    free(new);  //gmtk add (from if to else)for coverity 20090804
	    break;
	  }
		  ...
		  arg[2] == ':')
		{
		  if (arg[3] == '*')
			...
			len = (int) strlen(arg);
		      ...
		      new->clid_len = len;
		      //new->clid = safe_malloc(len);  //gmtk add (from if to else)for coverity 20090804
		      new->clid = safe_malloc(len+1);  //gmtk add (from if to else)for coverity 20090804
	...
	if (!arg)
	...
	else 
	  {
	    ...
	    free(dhcp_file);  //gmtk add (from if to else)for coverity 20090804  
	    free(dhcp_sname);  //gmtk add (from if to else)for coverity 20090804  
	  }
	...
	if (!(comma = safe_strchr(arg, ',')))
	...
	else
	  {
	    ...
	    *comma = 0;
	    new->netid.net = safe_string_alloc(arg);
	    unhide_metas(comma+1);
	    //new->len = strlen(comma+1);  //gmtk add (from if to else)for coverity 20090804
	    new->len = strlen(comma+2);  //gmtk add (from if to else)for coverity 20090804
      new->data = safe_malloc((new->len)+1);
...
static void one_file(struct daemon *daemon, char *file, int nest)	
{
  ...
  while (fgets(buff, MAXDNAME, f))
	...
	errmess = _("missing parameter");
      else if(arg)  //gmtk add (from if to else)for coverity 20090804
...
struct daemon *read_opts(int argc, char **argv, char *compile_opts)
{
  ...
  int option, nest = 0;
  char *errmess=NULL, *arg, *conffile = CONFFILE;
  int malloc_flag=0;  //gmtk add for coverity 20090806
  ...
  while (1) 
    ...
    malloc_flag = 1;  //gmtk add for coverity 20090805
	  nest++;
	}
      else
	{
#ifdef HAVE_GETOPT_LONG
    if(arg != NULL)  //gmtk add (from if to else)for coverity 20090804
  ...
  if(conffile && malloc_flag != 0)
    free(conffile);     //gmtk add for coverity 20090804
  return daemon;
}  
