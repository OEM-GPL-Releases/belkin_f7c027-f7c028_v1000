  ...
  if (cache_size > 0)
      ...
      for (i=0; i<size; i++, crecp++)
	{
	  ...
	  crecp->uid = uid++;
	   ...
	   free(crecp);  //gmtk add for Coverity 20090804
    }
			  ...
			  time_t now,  unsigned long ttl, unsigned short flags)
{
  ...
  {
    name = NULL;
    printf("DNSMASQ: this is in cache.c, variable name =NULL");  //gmtk add for coverity 20090804
    return 0; //gmtk add for coverity 20090804
  }
