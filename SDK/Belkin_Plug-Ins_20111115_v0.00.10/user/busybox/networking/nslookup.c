...
//+++GTK add for PPTP domain name type PPTP service address
	char ip_buf[16] ; 
	sprintf(ip_buf,"%d.%d.%d.%d",split[0], split[1], split[2], split[3]);

	FILE *pptp_fp;
	if( (pptp_fp = fopen("/tmp/pptp_server_ip" , "w")) == NULL )
	{
		printf("Can't write /tmp/pptp_server_ip for pptp \n");
	}	
	else
	{
		fprintf( pptp_fp , "%s\n",ip_buf);
