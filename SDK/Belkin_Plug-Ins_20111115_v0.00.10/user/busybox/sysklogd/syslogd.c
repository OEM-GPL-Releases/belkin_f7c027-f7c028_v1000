...
static void doSyslogd(void)
{
	...
	#define MY_PATH_LOG "/tmp/syslog"
	//#define MY_PATH_LOG _PATH_LOG
	...
	if (bind(sock_fd, (struct sockaddr *) &sunx, addrLength) < 0) {
		bb_perror_msg_and_die("Could not connect to socket " MY_PATH_LOG);
	}

	if (chmod(lfile, 0666) < 0) {
		bb_perror_msg_and_die("Could not set permission on " MY_PATH_LOG);
	}
